...
[type=openswitch name="OpenSwitch 2"] r1
...
@mark.test_id(1001)
def test_vxlan_rim(topology):
    """
    Test ping between 2 hosts connected through VxLAN tunnel
    between 2 VTEPs And a router in the middle (rim).

    [h1]-----[s1]-----[r1]-----[s2]------[h2]
    10.0.0.*    20.0.1.*  20.0.2.*  10.0.0.*

    Build a topology of a router, two switches, and two hosts. Put the router
    in the middle and connect the hosts to the switches.
    Setup a VxLAN between switches.
    Send nping traffic from host 1 to host 2.
    Assert we receive the traffic on host 2.
    """
    h1_ip = '10.0.0.10'
    h2_ip = '10.0.0.20'
    s1_tun_ip = '20.0.1.10'
    ...
    with r1.libs.vtysh.ConfigInterface('1') as ctx:
        ctx.ip_address(r1_s1_ip + all_mask)
        ctx.no_shutdown()

    with r1.libs.vtysh.ConfigInterface('2') as ctx:
        ctx.ip_address(r1_s2_ip + all_mask)
        ctx.no_shutdown()
    ...
    # configure the switch (static) routes
    with s1.libs.vtysh.Configure() as ctx:
        ctx.ip_route('20.0.2.0/24', r1_s1_ip)

    with s2.libs.vtysh.Configure() as ctx:
        ctx.ip_route('20.0.1.0/24', r1_s2_ip)
